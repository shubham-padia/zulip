...
class MessagePOSTTest(ZulipTestCase):
    ...
    def test_can_send_message_group_permission_for_streams(self) -> None:
        ...
        # send a message to that channel if they are part of both
        # `can_add_subscribers_group` and `can_send_message_group` for
        # that channel and `history_public_to_subscribers` is True.
        do_change_stream_group_based_setting(
            stream, "can_add_subscribers_group", guest_user_group, acting_user=othello
        )
        self._send_and_verify_message(guest_user, stream_name, allow_unsubscribed_sender=True)
