...
run_test("fast_track_current_msg_list_to_anchor date", ({override}) => {
    ...
    message_lists.current = list;
    $("#navbar-fixed-container")[0].getBoundingClientRect = () => ({height: 50});
    $("#top_of_feed_gutter")[0].getBoundingClientRect = () => ({height: 10});
    ...
    // 50px navbar + 10px box + the header's measured height (30px here).
