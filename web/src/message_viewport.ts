...
export function sticky_header_top(): number {
    // Recipient bars stick just below the top-of-feed gutter, which is
    // sticky at the bottom of the navbar and, like the navbar, hides
    // the message content that scrolls under it. This must match
    // the `top` of .message-feed .message_header in message_header.css,
    // which is where that offset is defined.
    //
    // Measured from bounding rects rather than computed styles, so that
    // the result is on the same sub-pixel grid as the rects of message
    // rows that callers compare it against; a computed style height is
    // rounded to four decimals.
    return (
        util.the($("#navbar-fixed-container")).getBoundingClientRect().height +
        util.the($("#top_of_feed_gutter")).getBoundingClientRect().height
    );
}
...
export function message_viewport_info(): MessageViewportInfo {
    ...
    let visible_top = sticky_header_top();
